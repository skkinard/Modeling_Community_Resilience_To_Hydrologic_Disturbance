...
slim_formula <- formula(
  ln_density~ depth_mx + ln_q2wk_max + ln_q2wk_med + ln_q2wk_sd + 
    sqrt_conductivity + sqrt_rain + temperature + width)


full_TR <- lm(slim_formula, data = d_fish%>%filter(site_code=='TR'))
step_TR <- stepAIC(full_TR, direction='both')
step_TR %>% tidy() %>% mutate(site_code = 'TR')

make_step_lm <- function(my_data) {
  lm(ln_biomass ~ width + ln_q2wk_max + ln_q2wk_med + 
               sqrt_rain + temperature, 
             data = my_data) %>% 
...
d_lm_step <- d_fish %>%
  group_by(site_code) %>%
  nest() %>%
  mutate(lm_step = map(data, make_step_lm)) %>%
  unnest(lm_step)

d_lm_step %>%
  add_rain() %>%
  fix_site_order() %>%
  mutate(is_signif = ifelse(p.value<0.1, 'yes', 'no')) %>%
  ggplot(aes(annualrain, estimate)) +
  facet_wrap(~term, scales='free') +
  dark_theme_grey(base_size=12) +
  geom_hline(yintercept=0, lty=2, lwd=.3, color='red') +
  geom_point(aes(color=is_signif), size = 3) +
  geom_line(lty=2,lwd=.3) +
  ggtitle('Within site, multiple regression coefficients: fish biomass ~')
  
d_fish %>%
  fix_site_order() %>%
  select(site_code, ln_density, width, sqrt_rain, ln_q2wk_max) %>%
  pivot_longer(cols=-any_of(c('site_code', 'ln_density')),
               names_to='xname', values_to='xvalue') %>%
  ggplot(aes(xvalue, ln_density)) +
