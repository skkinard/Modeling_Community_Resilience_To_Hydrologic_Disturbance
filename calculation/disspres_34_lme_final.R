...
make_slope_plot_qmax <- function(fit_model_engine) {
  ...
  my_output <- fit_model_engine$coefficients$random %>%
    ...
    geom_point(size=4, fill = 'cyan', shape = 21, alpha = .3) +
    geom_point(size=4, color = 'cyan', shape = 21, fill = NA) +
    labs(y=my_label,
         x='Annual Rainfall (cm)')
...
plot_lme_slope1 <- sig_plots$plot_slope[[1]]
plot_lme_slope2 <- sig_plots$plot_slope[[3]]
