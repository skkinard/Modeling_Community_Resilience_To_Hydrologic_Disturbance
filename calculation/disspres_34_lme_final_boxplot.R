...
make_slope_plot_qmax <- function(fit_model_engine) {
  ...
  my_output <- fit_model_engine$coefficients$random %>%
    ...
    rownames_to_column('site_code') %>%
    add_rain() %>%
    mutate(site_type = ifelse(annualrain<75, 'Semi-Arid', 'Semi-Humid')) %>%
    ggplot(aes(x=site_type, y=site_code.q_max)) +
    geom_hline(color='grey70', yintercept=0, lwd=.3) +
    geom_boxplot(aes(fill=site_type), alpha = .3) +
    geom_boxplot(aes(color=site_type), alpha = .3) +
    scale_fill_manual(values=c('gold', 'skyblue2')) +
    scale_color_manual(values=c('gold', 'skyblue2')) +
    theme(legend.position = 'none') +
    labs(y=my_label, x=element_blank())
    }
...
plot_lme_slopeboxplot1 <- sig_plots$plot_slope[[1]]
plot_lme_slopeboxplot2 <- sig_plots$plot_slope[[3]]


(plot_lme_slope2 + labs(x=element_blank()) +
    plot_lme_slopeboxplot2+labs(y=element_blank())) /
(plot_lme_slope2 + plot_lme_slopeboxplot2+labs(y=element_blank()))
